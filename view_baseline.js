...
perm_add_user_select = define_new_user_select_field(
  ...
  function (selected_user) {
    ...
    if (selected_user && selected_user.length > 0) {
      let userDropdown = $('#permdialog_user_select_dropdown');
...
perm_remove_user_button.click(function () {
  ...
  if (file_obj.using_permission_inheritance) {
    ...
    // Set username in the confirmation dialog
    are_you_sure_dialog.data('username', selected_username); // Storing selected username in dialog data for later use
    are_you_sure_dialog.dialog('open'); // Open the "are you sure" dialog
  }
});

// Adjusting the "Yes" button click event inside the are_you_sure_dialog definition
$('#are-you-sure-yes-button').click(function () {
  // Retrieve the stored username from dialog data
  let usernameToRemove = are_you_sure_dialog.data('username');
  let filepath = perm_dialog.attr('filepath');
  ...
  $(
    '#permdialog_user_select_dropdown option[value="' + usernameToRemove + '"]'
  ).remove();

  // Close the dialog
  are_you_sure_dialog.dialog('close');
});
...
define_attribute_observer(perm_dialog, 'filepath', function () {
  ...
  let file_users = get_file_users(path_to_file[current_filepath]);
  $.each(file_users, function (username, attributes) {
    userSelectDropdown.append(
      $('<option>', {
        value: username,
        text: attributes.displayName || username,
      })
    );
  });
  $('#permdialog_user_select_dropdown_container')
    .empty()
    .append(userSelectDropdown);

  // Initial user is set to the first user in the list
  if (userSelectDropdown.find('option').length > 0) {
    userSelectDropdown.val(userSelectDropdown.find('option:first').val());
  }

  // Whenever a user is selected, update the permissions checkboxes
  userSelectDropdown.change(function () {
    let selectedUser = $(this).val();
    grouped_permissions.attr('username', selectedUser);
  });

  userSelectDropdown.trigger('change');
});
